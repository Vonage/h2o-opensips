																	...
																	size_t len)
{
	...
	ret = tls_blocking_write(c, fd, buf, len, &tls_mgm_api, ssl_global_lock);
...
static void proto_tls_conn_clean(struct tcp_connection* c)
{
	...
	tls_conn_clean(c, ssl_global_lock);
}
...
send_it:
	...
	lock_get(&c->write_lock);
	n = tls_blocking_write(c, fd, buf, len, &tls_mgm_api, ssl_global_lock);
	lock_release(&c->write_lock);
...
static int tls_read_req(struct tcp_connection* con, int* bytes_read)
{
	...
	ret=tls_fix_read_conn(con, ssl_global_lock);
